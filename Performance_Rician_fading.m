...
for(i1=1:length(SNR))
    ...
    for(i2=1:channels_per_SNR)          % This loop can be avoided using Matrix notation
        ...
        if(AWGN_channel == 1)
            ...
            if(Rician_channel == 1)
                h = ( s*randn(1,1) + mu ) + j*( s*randn(1,1) + mu );
 %               ((randn(1,1).*sqrt(K).*cos(X.*cos(theta_0) + phi_0))./sqrt(1+K)) + j.*((randn(1,1).*sqrt(K).*cos(X.*cos(theta_0) + phi_0))./sqrt(1+K));    % Rician channel
            else
...
semilogy(SNRdB, BER)
hold on
BER = berfading(SNRdB,'psk',8,1,K);
semilogy(SNRdB, BER)

legend('Simulation','Theory','Location','Best')
title('Average Bit Error Rate - QPSK in Rician Fading')
xlabel('SNR[dB]')
ylabel('BER')
grid
axis([0 20 10^-3 1])
